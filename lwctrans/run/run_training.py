...
from lwctrans.paths import nnUNet_preprocessed
from lwctrans.run.load_pretrained_weights import load_pretrained_weights
from lwctrans.training.nnUNetTrainer.nnUNetTrainer import nnUNetTrainer
from lwctrans.utilities.dataset_name_id_conversion import maybe_convert_to_dataset_name
from lwctrans.utilities.find_class_by_name import recursive_find_python_class
...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    nnunet_trainer = recursive_find_python_class(join(lwctrans.__path__[0], "training", "nnUNetTrainer"),
                                                trainer_name, 'lwctrans.training.nnUNetTrainer')
    if nnunet_trainer is None:
        raise RuntimeError(f'Could not find requested nnunet trainer {trainer_name} in '
                           f'lwctrans.training.nnUNetTrainer ('
                           f'{join(lwctrans.__path__[0], "training", "nnUNetTrainer")}). If it is located somewhere '
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, outpath=outpath, fold=fold,
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, outpath, fold, trainer_class_name,
...
def run_training_entry():
    ...
    parser.add_argument('-d', '--dataset_name_or_id', type=str, default='402',
                        help="Dataset name or ID to train with")
    parser.add_argument('-configuration', type=str, default='3d_fullres',
                        help="Configuration that should be trained, 3d_fullres or 2d")
    parser.add_argument("-outpath", type=str, default='LW-CTrans', help='output path')
    ...
    parser.add_argument("-gpu_id", type=str, default='0',
                        help='Specify the ids of GPUs to use for training')
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.outpath, args.fold, args.tr, args.p, args.pretrained_weights,
