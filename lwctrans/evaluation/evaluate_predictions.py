...
from lwctrans.imageio.reader_writer_registry import determine_reader_writer_from_dataset_json, \
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    for r in labels_or_regions:
        ...
        test_empty = not np.any(mask_pred)
        test_full = np.all(mask_pred)
        reference_empty = not np.any(mask_ref)
        reference_full = np.all(mask_ref)
        if tp + fp + fn == 0:
        ...
        if test_empty or test_full or reference_empty or reference_full:
            ...
            results['metrics'][r]['HD95'] = hd95(mask_pred.squeeze(), mask_ref.squeeze(), spacing)
            results['metrics'][r]['ASSD'] = assd(mask_pred.squeeze(), mask_ref.squeeze(), spacing)
...
def compute_metrics_on_folder(folder_ref: str, folder_pred: str, output_file: str,
    ...
    for r in regions_or_labels:
        ...
        for m in metric_list:
            mean = np.nanmean([i['metrics'][r][m] for i in results])
            std = np.nanstd([i['metrics'][r][m] for i in results])
            if m == 'HD95' or m == 'ASSD':
                means[r][m] = r'{:.2f}±{:.2f}'.format(mean, std)
            else:
                means[r][m] = r'{:.2f}±{:.2f}'.format(mean * 100, std * 100)
    ...
    for m in metric_list:
        ...
        for k in means.keys():
            ...
            v_means.append(float(means[k][m].split('±')[0]))
            v_stds.append(float(means[k][m].split('±')[1]))
        foreground_mean[m] = r'{:.2f}±{:.2f}'.format(np.mean(v_means), np.mean(v_stds))
